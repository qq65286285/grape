...
public class DocumentSplitter {
     ...
     * @return 解析后的测试场景对象
     */
    private TestScenario parseSingleScenario(String scenarioText) {
        ...
        for (ScenarioTypeEnum type : types) {
            // 为每个场景类型创建对应的内容列表
            // 注意：这里简单地将所有内容分配给每个类型
            // 实际应用中可能需要根据内容特征分配到不同类型
     ...
     * @return 场景名称
     */
    private String extractScenarioName(String scenarioText) {
        // 从文本中提取场景名称
     ...
     * @return 场景类型列表
     */
    private List<ScenarioTypeEnum> extractScenarioTypes(String scenarioText) {
     ...
     * @param types 类型列表
     */
    private void addTypesFromString(String typeStr, List<ScenarioTypeEnum> types) {
        // 支持多种分隔符
     ...
     * @return 推断的场景类型
     */
    private ScenarioTypeEnum inferScenarioType(String scenarioText) {
        // 直接使用枚举类中的推断方法
     ...
     * @return 场景内容列表
     */
    private List<ScenarioContent> extractScenarioContents(String scenarioText) {
